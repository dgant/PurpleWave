...
object VisualizeScheduler {
  ...
  def render() {
    ...
    DrawScreen.table(0, 100, With.scheduler.queue.take(20).map(buildable => List(buildable.toString)))
    With.game.drawTextScreen(150, 85, "Next to build")
    DrawScreen.table(150, 100, With.scheduler.simulationResults.suggestedEvents
      .toList
    ...
    DrawScreen.table(300, 100, With.scheduler.simulationResults.simulatedEvents
    ...
    DrawScreen.table(500, 100, With.scheduler.simulationResults.unbuildable
      .toList
      .take(20)
