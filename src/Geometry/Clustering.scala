...
object Clustering {
    ...
    things.foreach(thing => {
      if ( ! unitLeaders.contains(thing)) {
        ...
        groupsByLeader(thing) ++= neighborsByUnit(thing).filter(neighbor =>
          ! limitRegion ||
            With.grids.altitudeBonus.get(extractPosition(thing).toTilePosition) ==
            With.grids.altitudeBonus.get(extractPosition(neighbor).toTilePosition))
  ...
  private def mapUnitsToNeighbors[T](
    ...
    things.map(thing => (thing, things.filter(extractPosition(_).distancePixelsSquared(extractPosition(thing)) <= radiusSquared))).toMap
  }
}
