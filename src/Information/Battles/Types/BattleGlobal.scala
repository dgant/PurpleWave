...
class BattleGlobal(us: Team, enemy: Team) extends Battle(us, enemy) {
  ...
  lazy val valueEnemyArmy   : Double = sumValue(With.units.enemy.filter(u => u.unitClass.helpsInCombat))
  lazy val valueUsArmy      : Double = sumValue(With.units.ours.filter(u => u.unitClass.helpsInCombat))
  lazy val valueUsArmyKnown : Double = sumValue(With.units.ours.filter(u => u.unitClass.helpsInCombat && u.knownToEnemy))
  lazy val valueRatioTarget : Double = Math.min(0.5, PurpleMath.nanToOne(valueEnemyArmy / (valueEnemyArmy + valueUsArmyKnown)))
