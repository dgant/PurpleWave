...
import Information.Grids.{AbstractTypedGrid, GridUpdateUnit}
...
/**
  ...
  */
abstract class AbstractGridFloody extends AbstractTypedGrid[Int] with GridUpdateUnit {
  final override val defaultValue: Int = 0
  @inline final def getUnchecked(i: Int): Int = tiles(i).maxDepth
  @inline final def inRange         (tile : Tile) : Boolean = ?(tile.valid, inRangeUnchecked(tile), false)
  @inline final def inRange         (i    : Int)  : Boolean = ?(valid(i),   inRangeUnchecked(i),    false)
  ...
  final def updateUnit(unit: UnitInfo): Unit = {
    if (shouldInclude(unit)) {
      ...
      if (floodyOld.isDefined) {
        val old = floodyOld.get
        if (unit.tile != old.tile || unitRadius != old.radius) {
          removeUnit(old)
          addUnit(floodyNew)
        }
      } else {
  ...
  @inline private final def flood(floody: FloodyUnit): Seq[(Tile, Int)] = {
    ...
    (0 to max).view.flatMap(d => Ring(d).map(p => (floody.tile.add(p), max - d))).filter(_._1.valid)
  }
}
