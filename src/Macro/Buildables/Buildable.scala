...
abstract class Buildable(val buildableType: BuildableType, val quantity: Int = 0) extends {
  ...
  def unitsRequired     : Iterable[UnitClass]     = unit.map(_.requiredUnits.keys).orElse(upgrade.map(u => Seq(u.whatsRequired(quantity)))).getOrElse(tech.map(_.requiredUnit).toSeq).filterNot(UnitClasses.None==)
