...
object Cancel extends Action {
  ...
  override def allowed(unit: FriendlyUnitInfo): Boolean = {
    ...
    lazy val doomed           = unit.hitPoints < unit.unitClass.maxHitPoints && framesToLive < framesCutoff && (unit.damageQueue.nonEmpty || unit.matchups.threatsInRange.exists(_.presumptiveTarget.contains(unit)))
