...
object Avoid extends ActionTechnique {
  ...
  override def perform(unit: FriendlyUnitInfo) {
    val threatBonus     = PurpleMath.clamp(With.reaction.agencyAverage + unit.matchups.framesOfEntanglementDiffused, 12.0, 24.0) / 12.0
    val exitBonus       = if (unit.agent.origin.zone == unit.zone) 0.25 else 1.0
    val regroupingBonus = 18.0 / Math.max(24.0, unit.matchups.framesOfEntanglementDiffused)
