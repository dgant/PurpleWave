...
object KnockKnock extends Action {
  ...
  override def allowed(unit: FriendlyUnitInfo): Boolean = (
    unit.agent.isScout
    && (unit.zone != unit.agent.destination.zone || unit.agent.destination.zone.edges.exists(_.contains(unit.pixel)))
    && unit.matchups.threats.size == 1
    && unit.matchups.threats.forall(threat =>
      unit.canAttack(threat)
      && threat.unitClass.isWorker
      && unit.agent.destination.zone.edges.exists(_.contains(threat.pixel))
