...
class Agent(val unit: FriendlyUnitInfo) {
  ...
  private val originCache = new KeyedCache(
    () =>
      ...
      .orElse(unit.matchups.anchor.filter(_.matchups.threats.nonEmpty).map(a => a.pixel.project(unit.battle.map(_.enemy.centroidOf(unit)).getOrElse(a.pixel), AnchorMargin())))
      .orElse(toReturn)
      .orElse(
        ByOption.minBy(
