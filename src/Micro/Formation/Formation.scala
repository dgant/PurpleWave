  ...
  def renderMap(): Unit = {
    placements.foreach(p => {
      ...
      val box = Seq(
        ...
        slot.add(   c.dimensionRightInclusive,  - c.dimensionUp),
        slot.add(   c.dimensionRightInclusive,    c.dimensionDownInclusive),
        slot.add( - c.dimensionLeft,              c.dimensionDownInclusive)).map(_.subtract(style.offset, style.offset))
    ...
    DrawMap.polygon(Maff.convexHull(placements.flatMap(p => p._1.unitClass.cornersInclusive.map(p._2.add)).toSeq), style.color)
  }
