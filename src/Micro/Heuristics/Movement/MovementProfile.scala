...
case class MovementProfile(
  ...
  var preferDistanceFromEdge  : Double = 0.0,
  ...
  def this(source: MovementProfile) {
    this(
      preferVpfDealing        = source.preferVpfDealing,
      preferMobility          = source.preferMobility,
      preferDistanceFromEdge  = source.preferDistanceFromEdge,
      avoidVpfReceiving       = source.avoidVpfReceiving,
      avoidRetreatFrames      = source.avoidRetreatFrames,
      avoidDamage             = source.avoidDamage,
      avoidTraffic            = source.avoidTraffic,
      avoidShovers            = source.avoidShovers)
  }
  ...
  def weightedHeuristics: Iterable[MovementHeuristicWeight] =
    Vector(
      ...
      new MovementHeuristicWeight(MovementHeuristicDistanceFromEdge,        preferDistanceFromEdge, Colors.BrightGray),
