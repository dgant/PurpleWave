...
case class MatchupAnalysis(
  ...
  lazy val threatsInRange : Vector[UnitInfo]          = threats.filter(threat => threat.pixelRangeAgainstFromCenter(us) >= threat.pixelDistanceFast(at) - us.pixelsTravelledMax(frame) - threat.pixelsTravelledMax(frame))
  lazy val targetsInRange : Vector[UnitInfo]          = targets.filter(target => us.pixelRangeAgainstFromCenter(target) >= target.pixelDistanceFast(at) - us.pixelsTravelledMax(frame) - target.pixelsTravelledMax(frame))
  ...
  lazy val framesToRetreatDiffused    : Double  = if (threatsInRange.isEmpty) 0.0 else threatsInRange.map(threat => us.framesToTravelPixels(threat.pixelRangeAgainstFromCenter(us) - threat.pixelDistanceFast(at))).max
