...
class ReactionTimes {
  ...
  def planningLast        : Int = With.framesSince(With.priorities.lastResetFrame)
  ...
  private val planningMaxCache          = new Cache(() => Maff.max(With.priorities.frameDelays).getOrElse(0))
  ...
  private val planningAverageCache      = new Cache(() => filterTimes(With.priorities.frameDelays).sum     / Math.max(1, filterTimes(With.priorities.frameDelays).size))
}
