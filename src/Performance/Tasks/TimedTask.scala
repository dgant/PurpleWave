  ...
  final def run(budgetMs: Long) {
    ...
    if ( ! limitAlreadyViolated && With.performance.violatedLimit) {
      ...
      if (skipsMax > 0 && With.configuration.enablePerformancePauses && With.performance.lastTaskWarningFrame < With.frame) {
        ...
        With.logger.warn(f"$toString${if(due)" (Due)" else ""} crossed ${With.configuration.frameLimitMs}ms taking ${millisecondsDuration}ms on a ${budgetMs}ms budget (${Maff.meanL(budgetMsPast).toInt}ms avg budget), reaching ${With.performance.frameMs}ms on the frame.")
      }
    }
