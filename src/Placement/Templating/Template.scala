...
class Template extends TemplateFilter {
  ...
  def add(point: Point, requirement: TemplatePointRequirement): Template = {
    ...
    Maff.sortStablyInPlaceBy(points)(p => ?(p.requirement.isTownHall, 0, ?(p.requirement.isGas, 1, 2)))
    left.invalidate()
    right.invalidate()
    top.invalidate()
    bottom.invalidate()
    ...
    while (y < strings.length) {
      ...
      while (x < string.length) {
        ...
        val pt: TemplatePointRequirement = char.toUpper match {
          ...
          case 'S' => new TemplatePointRequirement(Protoss.Stargate, Terran.EngineeringBay).withLabels(Tech)
          ...
          case 'X' => null
          case 'Y' => new TemplatePointRequirement(Terran.SupplyDepot, Terran.Academy, Terran.Armory)
          case 'Z' => null
          case '0' => null
          case '1' => null
