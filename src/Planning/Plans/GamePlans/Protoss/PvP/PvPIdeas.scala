...
import Planning.Plans.GamePlans.Protoss.PvP.PvPTools.{get, pump, status}
...
object PvPIdeas extends MacroCounting {
  ...
  def shouldAttack: Boolean = {
    ...
    output &&= unitsComplete(Protoss.Observer) > 0 || ! enemyHasShown(Protoss.DarkTemplar) || (unitsComplete(Protoss.DarkTemplar) > 0 && enemiesComplete(Protoss.PhotonCannon, Protoss.Observer) == 0)
    output
  }
