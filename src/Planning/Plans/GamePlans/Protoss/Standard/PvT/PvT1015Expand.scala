...
import Planning.UnitMatchers.{MatchAnd, MatchInNatural, MatchSiegeTank}
...
class PvT1015Expand extends GameplanTemplate {
  ...
  override def scoutPlan = new If(new UpgradeStarted(Protoss.DragoonRange), new ScoutWithWorkers)
    ...
    new If(
      new Or(
        ...
        new And(
          ...
          new EnemiesAtMost(0, MatchSiegeTank))),
