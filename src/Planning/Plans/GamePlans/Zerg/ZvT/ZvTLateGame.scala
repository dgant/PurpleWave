...
class ZvTLateGame extends GameplanTemplate  {
  ...
  override def buildPlans: Seq[Plan] = Seq(
    ...
    new If(
      new EnemiesAtLeast(1, MatchTank),
      new Parallel(
    ...
    new PumpRatio(Zerg.Zergling, 12, 30, Seq(Enemy(Terran.Marine, 3), Enemy(Terran.Firebat, 5), Enemy(Terran.Goliath, 6), Enemy(MatchTank, 8))),
