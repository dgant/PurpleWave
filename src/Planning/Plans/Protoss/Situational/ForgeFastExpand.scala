...
class ForgeFastExpand(var cannonsInFront: Boolean) extends ProposePlacement {
  override lazy val blueprints: Iterable[Blueprint] = Vector(
    new Blueprint(this, building = Some(Protoss.Pylon),           requireZone = With.geography.ourNatural.map(_.zone), placementProfile = Some(PlacementProfiles.naturalCannonPylon), preferredDistanceFromExit = Some(if (cannonsInFront) 5.0 else 0.0), preferWallPlacement = Some(true)),
    new Blueprint(this, building = Some(Protoss.Forge),           preferZone = With.geography.ourNatural.map(_.zone), placementProfile = Some(PlacementProfiles.naturalCannonPylon), preferredDistanceFromExit = Some(if (cannonsInFront) 5.0 else 0.0), preferWallPlacement = Some(true)),
