...
class ForgeFastExpand(var cannonsInFront: Boolean) extends ProposePlacement {
  override lazy val blueprints: Iterable[Blueprint] = Vector(
    ...
    new Blueprint(this, argWall = Some(true), argMargin = Some(false),  argPlacement = Some(PlacementProfiles.naturalCannon),      building = Some(Protoss.PhotonCannon),  zone = With.geography.ourNatural.map(_.zone), argRangePixels = Some(2.0 * 32.0)),
    new Blueprint(this, argWall = Some(true), argMargin = Some(false),  argPlacement = Some(PlacementProfiles.naturalCannon),      building = Some(Protoss.PhotonCannon),  zone = With.geography.ourNatural.map(_.zone), argRangePixels = Some(2.0 * 32.0)),
