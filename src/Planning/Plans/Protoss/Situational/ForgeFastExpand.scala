...
class ForgeFastExpand(var cannonsInFront: Boolean) extends ProposePlacement {
  override lazy val blueprints: Iterable[Blueprint] = Vector(
    new Blueprint(this, argWall = Some(true),                           argPlacement = Some(PlacementProfiles.naturalCannonPylon), building = Some(Protoss.Pylon),         zone = With.geography.ourNatural.map(_.zone), argRangePixels = Some(if (cannonsInFront) 5.0 else 0.0)),
    new Blueprint(this, argWall = Some(true),                           argPlacement = Some(PlacementProfiles.naturalCannonPylon), building = Some(Protoss.Forge),         zone = With.geography.ourNatural.map(_.zone), argRangePixels = Some(if (cannonsInFront) 5.0 else 0.0)),
